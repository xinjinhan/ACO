            ...
            if self.open_table_city[i]:
                try:
                    ...
                    if possible_choice_of_path_with_transport[0] == 0 and possible_choice_of_path_with_transport[1] == 0 \
                        ...
                        continue
                    select_citys_prob[i] = pow(pheromone_graph[self.current_city][i], ALPHA) * pow(
                        (1.0 / min(abs(filter(lambda x: x != 0, possible_choice_of_path_with_transport)))), BETA) * pow(
                        (1.0 / min(abs(filter(lambda x: x != 0, possible_choice_of_path_with_transport_carbon)))), BETA)
                    total_prob += select_citys_prob[i]
                except ZeroDivisionError as e:
                    print('Ant ID: {ID}, current city: {current}, target city: {target}'.format(ID=self.ID,
                                                                                                current=self.current_city,
        ...
        while self.__running:
            while self.current_sampling_times != self.total_sampling_times:
                ...
                for ant in self.ants:
                    ...
                    ant.search_path()
                    # 与当前最优蚂蚁比较
                    if ant.total_cost < self.best_ant.total_cost and ant.total_carbon < carbon_max:
                        # 更新最优解
                ...
                # 设置标题
                self.title("ACO-多式联运-碳强制 (n:随机初始 e:开始搜索 s:停止搜索 q:退出程序) 迭代次数: %d"
